...
export default {
  ...
  execute: async function (message, bot) {
    let logMessage = true; // this is set to false if logging should be skipped
    ...
    try {
      ...
      if (bot.config.showMcChat && msgType === SenderType.Minecraft) {
        ...
        if (logMessage === "redactQuiz")
          ...
          return bot.utils.webhookLogger.addMessage(
            message
              .toAnsi(undefined, bot.utils.discordAnsiCodes)
              .replace(/(\!p \w+ .+\|\s*)\S.*(\[0m)$/, "$1<redacted>$2"),
      ...
      } else if (msgType === SenderType.Console) {
        bot.utils.webhookLogger.addMessage(
          message,
          bot.utils.classifyMessage(message.toString()),
        );
